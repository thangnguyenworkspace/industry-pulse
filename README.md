...
```bash
git clone https://github.com/thangnguyenworkspace/industry-pulse.git
cd industry-pulse
...
To make it fully yours, fork the repo first and clone your fork; the snapshot is meant to be taken and adapted, not tracked.
...
The operational limits (third-party actor dependence, on-demand-only, lanes dropped at the gate) are covered under [Cost and limitations](#-cost-and-limitations). What follows is what Pulse deliberately is not:
...
The reusable core, crawl to classify to brief, is useful on its own, and its full design is documented in [docs/architecture.md](docs/architecture.md). The layer that made it personal, how I score and deliver briefs, stays private. What is here is the part anyone can build on.
...
```
.
├── .claude/skills/      # The pipeline: run-pulse + four crawl leaves (Agent Skills format)
...
The capability payload lives under `.claude/skills/<name>/SKILL.md` (the Agent Skills format: frontmatter plus an auto-discovered body). Sibling repos in this collection that ship slash-commands instead use `.claude/commands/<name>.md`; the layout differs because the mechanism differs, and each is the native form for what it holds.
