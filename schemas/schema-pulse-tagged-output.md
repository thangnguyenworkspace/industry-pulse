...
|-------|------|----------|---------|------------|
| type | String | Yes |n/a | Pulse Tagged Output |
| schema | String | Yes |n/a | pulse-tagged-output |
| created | Date | Yes |n/a | YYYY-MM-DD, matches `tagged_date`. |
| updated | Date | Yes |n/a | YYYY-MM-DD, matches `created` (files are immutable per-run intermediates). |
...
| description | String | Yes |n/a | One-line summary of the source-type + dominant domains tagged. |
| source_type | String | Yes |n/a | Enum §3.2, the single source-type this file covers. File-level by path construction. |
| tagged_date | Date | Yes |n/a | YYYY-MM-DD, run date; matches the `tagged-{date}/` folder + filename suffix. |
...
| profile_version_consumed | Number | Yes |n/a | `config/profile.md` `version:` whose §3.0 Domain Interests enum the tagging ran against. |
...
|-------|------|----------|-------------|-------------|-------|
| H1 Title | Heading | Yes |n/a | replace | Exactly one H1, format `# Pulse Tagged Output, {source_type}, YYYY-MM-DD`. |
