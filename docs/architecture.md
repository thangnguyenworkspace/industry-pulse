...
There is no L2. A per-domain synthesis layer was considered and dropped — the main thread does cross-domain synthesis directly from the tagged files, and the token math did not justify the extra hop. The numbering is kept from the internal design so the layers read consistently.
